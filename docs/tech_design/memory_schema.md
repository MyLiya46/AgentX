...
- `status`（SMALLINT，默认 1）
  ...
  - **软删除策略**：`memory_items` 表使用 `status` 字段作为软删除标记（而非 `deleted_at`），
    因为该表有独立的生命周期语义（active/archived），与通用逻辑删除（deleted_at 时间戳）不同。
    读取查询需显式过滤 `status = 1`，删除操作通过 `UPDATE SET status = 0` 实现。
- `deleted_at`（TIMESTAMPTZ，可空）
  - 含义：通用审计列（继承自 `BaseEntity`），记录删除时间戳。
  - **注意**：`MemoryItemEntity` 不使用 MyBatis-Plus `@TableLogic`（逻辑删除由 `status` 字段管理），
    因此 `deleted_at` 仅作为常规可空审计字段存在，查询不会自动注入 `WHERE deleted_at IS NULL`。
...
CREATE TABLE IF NOT EXISTS public.memory_items (
    ...
    deleted_at         TIMESTAMPTZ,                          -- 通用删除审计列
...
- 新增实体：`src/main/java/org/xhy/domain/memory/model/MemoryItemEntity.java`
  - `@TableName("memory_items")`，继承 `BaseEntity`（含 `createdAt`、`updatedAt`、`deletedAt` 审计字段）。
  - 自定义字段：`id,userId,type,text,data,importance,tags,sourceSessionId,dedupeHash,status`。
  - **重要**：不使用 `@TableLogic` 逻辑删除（由 `status` 字段管理软删除语义）。`deletedAt` 仅作为常规审计列存在。
